...
export default function AuthPage() {
    ...
    const [selectedRole, setSelectedRole] = useState('acheteur') // 'acheteur' | 'vendeur'
    ...
    const onSubmit = async (data) => {
        ...
        try {
            if (mode === 'signin') {
                ...
                // La redirection est gérée par AuthGuard dans App.jsx
            } else {
    ...
    return (
        <main className="flex h-screen w-full font-body overflow-hidden bg-surface text-on-surface">
            ...
            {/* ── RIGHT : Auth Form ────────────────────── */}
            <section className="w-full md:w-2/5 flex flex-col justify-center items-center p-8 md:p-16 bg-surface overflow-y-auto">
                <div className="w-full max-w-[400px]">

                    {/* Mobile logo */}
                    <div className="md:hidden mb-12 flex justify-center">
                        <img src={logoDarna} alt="Darna Logo" className="h-20 w-auto object-contain -ml-20" />
                    </div>

                    {/* Logo desktop */}
                    <div className="flex justify-end">
                        <img src={logoDarna} alt="Darna Logo" className="h-20 w-auto object-contain" />
                    </div>
                    ...
                    <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
                        ...
                        {/* Choix du rôle — signup only */}
                        {mode === 'signup' && (
                            <div>
                                <label className="block text-sm text-on-surface-variant mb-2 ml-1">
                                    Je suis un...
                                </label>
                                <div className="grid grid-cols-2 gap-3">
                                    ...
                                    <button
                                        type="button"
                                        onClick={() => setSelectedRole('acheteur')}
                                        className={`relative flex flex-col items-center gap-2 p-4 rounded-xl border-2 transition-all ${selectedRole === 'acheteur'
                                            ? 'border-primary bg-primary/5'
                                            : 'border-outline-variant/20 hover:border-outline-variant/50'
                                            }`}
                                    >
                                        <span className="text-2xl">🏠</span>
                                        <span className="text-sm font-semibold text-on-surface">Acheteur</span>
                                    ...
                                    <button
                                        ...
                                        className={`relative flex flex-col items-center gap-2 p-4 rounded-xl border-2 transition-all ${selectedRole === 'vendeur'
                                            ? 'border-primary bg-primary/5'
                                            : 'border-outline-variant/20 hover:border-outline-variant/50'
                                            }`}
                                    >
                                        <span className="text-2xl">🏢</span>
                                        <span className="text-sm font-semibold text-on-surface">Vendeur / Agence</span>
                                        <span className="text-[10px] text-on-surface-variant text-center leading-tight">
                                            Je publie des biens immobiliers
                                        </span>
                        ...
                        >
                            <svg className="w-5 h-5" viewBox="0 0 24 24">
                                ...
                                <path d="M12 5.38c1.62 0 3.06.56 4.21 1.66l3.15-3.15C17.45 2.09 14.97 1 12 1c-4.3 0-7.99 2.47-9.82 6.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335" />
                            </svg>
